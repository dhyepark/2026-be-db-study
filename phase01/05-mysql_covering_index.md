...
클러스터링 키 값(PK)이 변경될 때마다, 데이터 레코드의 주소가 변경되고(InnoDB에서 데이터가 PK순으로 정렬된 B-Tree에 저장되기 때문), 그때마다 해당 테이블의 모든 인덱스에 저장된 주솟값을 변경해 줘야 할 것이다.

주소가 바뀌는 건 PK가 변경될 때만이 아니다. 페이지가 꽉 찬 상태에서 새 레코드가 들어오면 페이지 분할이 일어나고, 이때 레코드들이 물리적으로 이동한다. PK 값은 그대로인데 주소만 바뀌는 것이다. 

주소를 저장하는 방식이었다면 INSERT 한 번 때문에 모든 인덱스를 따라가 고쳐야 한다.

이런 오버헤드를 제거하기 위해, **InnoDB 테이블(클러스터링 테이블)의 모든 세컨더리 인덱스는 해당 레코드가 저장된 주소가 아니라, 프라이머리 키 값을 저장하도록 구현돼 있다.**

PK 17번은 페이지가 어떻게 쪼개지든 여전히 PK 17번이다. 그래서 인덱스 키 컬럼도 PK도 바뀌지 않는 UPDATE라면, 레코드가 물리적으로 어디로 옮겨가든 세컨더리 인덱스는 손댈 필요가 없다.
